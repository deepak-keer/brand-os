...
export default function Analytics() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="font-display font-bold text-xl text-[#e8e8f0]">Analytics</h1>
      ...
      <div className="card overflow-hidden p-0">
        <div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom:'1px solid #1e1e2a' }}>
          ...
          <button className="btn-primary text-[12px] py-1.5" onClick={() => { setEditPlatform(null); setForm({ platform:'YouTube',followers:0,totalViews:0,totalLikes:0,totalComments:0,engagementRate:0 }) }}>
            + Add Platform
          </button>
        </div>
          ...
          <tbody>
            {latestByPlatform.map(a => (
              <tr key={a._id}>
                ...
                <td><button className="btn-ghost text-[11px]" onClick={() => { setEditPlatform(a.platform); setForm({ platform:a.platform, followers:a.followers, totalViews:a.totalViews, totalLikes:a.totalLikes, totalComments:a.totalComments||0, engagementRate:a.engagementRate }) }}>Update</button></td>
              </tr>
            ))}
